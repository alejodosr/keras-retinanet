    ...
    if inputs is None:
        if keras.backend.image_data_format() == 'channels_first':
            if static_batch_size:
                # inputs = keras.layers.Input(batch_shape=(64, 3, 800, 1069))
                inputs = keras.layers.Input(shape=(3, 800, 1069), batch_size=64, dtype=tf.float32)
            else:
                inputs = keras.layers.Input(shape=(3, None, None))
        else:
            if static_batch_size:
                # inputs = keras.layers.Input(batch_shape=(64, 800, 1069, 3))
                inputs = keras.layers.Input(shape=(800, 1069, 3), batch_size=64, dtype=tf.float32)
            else:
